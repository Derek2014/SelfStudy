...
function Update ()
	...
	if (Input.GetKeyDown(KeyCode.E) && Payload)
		Payload.GetComponent.<Rigidbody>().AddForce(Vector3(0, 6, 0), ForceMode.VelocityChange);
	...
	// Teclas de configuraci�n del coche
	...
	if (Input.GetKeyDown(KeyCode.F12))
		{
		var bQuality = MainLight.GetComponent.<Light>().shadows != LightShadows.None;
...
static function DisableCar(Car : CarControl)
	...
	// Asegurarse de mantener la configuraci�n apropiada
...
static function SendInput(Car : CarControl, reverseRequiresStop : boolean, brakeRelease : boolean) : boolean
	...
	var speedForward = Vector3.Dot(Car.GetComponent.<Rigidbody>().velocity, Car.transform.forward);
	var speedSideways = Vector3.Dot(Car.GetComponent.<Rigidbody>().velocity, Car.transform.right);
	var speedRotation = Car.GetComponent.<Rigidbody>().angularVelocity.magnitude;
	...
	if (reverseRequiresStop)
		{
		// Determinar la marcha a meter (adelante - detr�s)
		// Las marchas van en funci�n de las acciones sobre el eje vertical (forward - reverse)
		...
		if (brakeRelease)
			...
			if (speedSideways < GEARSPEEDMIN && speedRotation < GEARSPEEDMIN)
				{
				// Hacer que entre la marcha adecuada con el coche movi�ndose por inercia longitudinalmente.
		...
		{
		// Modo adelante-atr�s sin detenerse (por Sagron)
...
function OnGUI ()
	...
	var SpeedMS = Vector2(Vector3.Dot(Car.GetComponent.<Rigidbody>().velocity, Car.transform.forward), Vector3.Dot(Car.GetComponent.<Rigidbody>().velocity, Car.transform.right)).magnitude;
