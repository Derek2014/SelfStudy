...
public abstract class FingerGestures : MonoBehaviour
{
    ...
    public class Finger
    {
        ...
        internal void Update( FingerPhase newPhase, Vector2 newPos )
        {
            ...
            if( phase != newPhase )
            {
                ...
                {
                    Watchdog.LogWarning( "Correcting bad FingerPhase transition (FingerPhase.Ended skipped)" );
                ...
                {
                    Watchdog.LogWarning( "Correcting bad FingerPhase transition (FingerPhase.Began skipped)" );
                ...
                {
                    Watchdog.LogWarning( "Invalid state FingerPhase transition from " + phase + " to " + newPhase + " - Skipping." );
                    return;
                }
            }
            else // same phase as before
            {
                ...
                {
                    Watchdog.LogWarning( "Duplicated FingerPhase." + newPhase.ToString() + " - skipping." );
                    return;
                }
            }
	...
	public bool OnResume(){
		Watchdog.Log("[FingerGesture] onResume");
		instance = null;
		OnEnable();
		return true;
	}
    ...
    protected virtual void OnEnable()
    {
        ...
        {
            Watchdog.LogWarning( "There is already an instance of FingerGestures created (" + instance.name + "). Destroying new one." );
